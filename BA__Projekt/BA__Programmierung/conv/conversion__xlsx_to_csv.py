...
def process_datasets(base_input_dir, base_output_dir):
    ...
    for subdir in os.listdir(base_input_dir):
        ...
        if os.path.isdir(subdir_path) and subdir.startswith("dataset__"):
            ...
            # Look for .xlsx file in this dataset__ folder
            for file in os.listdir(subdir_path):
                if file.endswith(".xlsx"):
                    input_xlsx_path = os.path.join(subdir_path, file)
                    ...
                    convert_xlsx_to_csv(input_xlsx_path, output_csv_path)
                    break  # Only convert the first .xlsx file found
            else:
                print(f"⚠️  No .xlsx file found in {subdir_path}")
