...
class TestEvidentialRegressionNavalPropulsion(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.device = "cpu"
        cls.dataset = NavalPropulsionDataset(
            db_path="/root/BA__U-i-mlb-Sm-f-d-s-V-a-S/BA__Projekt/assets/dbs/dataset__naval_propulsion__dataset.duckdb",
            table_name="naval_propulsion__dataset_csv"
        )
        cls.loader = DataLoader(cls.dataset, batch_size=32, shuffle=False)
        cls.model = EvidentialNet(input_dim=16).to(cls.device)  # Naval dataset has 16 input features
        cls.model.eval()
