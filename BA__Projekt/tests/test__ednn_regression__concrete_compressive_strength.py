# BA__Projekt/tests/test__ednn_regression__concrete_compressive_strength.py
...
class TestEvidentialRegressionConcreteCompressiveStrength(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.device = "cpu"
        cls.dataset = DatasetTorchConcreteCompressiveStrength(
            db_path="/root/BA__U-i-mlb-Sm-f-d-s-V-a-S/BA__Projekt/assets/dbs/dataset__concrete_compressive_strength__dataset.duckdb",
            table_name="concrete_compressive_strength__dataset_csv"
        )
        cls.loader = DataLoader(cls.dataset, batch_size=32, shuffle=False)
        cls.model = EvidentialNet(input_dim=8).to(cls.device)  # Dataset has 8 input features
        cls.model.eval()
